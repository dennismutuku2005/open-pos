...
export function Modal({
    ...
    const typeConfig = {
        primary: {
            ...
            btn: 'bg-openpos-purple hover:bg-openpos-purple/90 shadow-openpos-purple/20'
        },
        info: {
            icon: Info,
            color: 'text-blue-500',
            ...
            <div className={cn("relative bg-card-bg w-full rounded-3xl shadow-2xl border border-openpos-border animate-in zoom-in-95 fade-in duration-300 max-h-[90vh] flex flex-col transition-colors duration-300", maxWidth)}>
                <div className="flex-1 overflow-y-auto custom-scrollbar p-6 sm:p-8 pb-20">
                    <div className="flex items-start justify-between mb-4">
                        ...
                        <button
                            onClick={onClose}
                            className="p-2 hover:bg-openpos-bg-subtle rounded-xl text-admin-dim hover:text-foreground transition-all"
                        >
                            <X size={18} />
                        </button>
                    </div>
                                ...
                                <>
                                    {showCancel && (
                                        <button
                                            onClick={onClose}
                                            disabled={isLoading}
                                            className="flex-1 px-4 py-3 bg-card-bg border border-openpos-border text-admin-dim rounded-xl text-[10px] font-bold uppercase tracking-widest hover:bg-openpos-bg-subtle transition-all disabled:opacity-50"
                                        >
                                            {cancelText}
                                        </button>
                                    )}
