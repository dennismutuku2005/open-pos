            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                <div className="flex flex-col gap-2">
                    ...
                    <h1 className="text-xl font-bold tracking-tight text-admin-value uppercase flex items-center gap-2">
                        <ShoppingBag size={20} className="text-openpos-blue" />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                >
                    <div className="space-y-2 mt-4">
                        {topProducts.map((p, i) => (
                            <div key={p.id} className="flex items-center justify-between p-4 rounded-2xl bg-openpos-bg-subtle/30 border border-openpos-border hover:border-openpos-blue/30 transition-all group cursor-default">
                                <div className="flex items-center gap-4">
                                    <div className="w-10 h-10 rounded-xl bg-openpos-blue/5 text-openpos-blue border border-openpos-blue/10 font-bold flex items-center justify-center text-[12px] group-hover:scale-105 transition-transform">
                                        #{i + 1}
                                    </div>
                                    <div>
                                        <p className="text-[13px] font-bold text-admin-value uppercase tracking-tight group-hover:text-openpos-blue transition-colors">{p.name}</p>
                ...
                >
                    <div className="space-y-2 mt-4">
                        {leastProducts.map((p, i) => (
                            <div key={p.id} className="flex items-center justify-between p-4 rounded-2xl bg-openpos-bg-subtle/30 border border-openpos-border hover:border-red-400/30 transition-all group cursor-default">
                                <div className="flex items-center gap-4">
                                    <div className="w-10 h-10 rounded-xl bg-red-500/5 text-red-400 border border-red-500/10 font-bold flex items-center justify-center text-[12px] group-hover:scale-105 transition-transform">
                                        #{i + 1}
                                    </div>
                                    <div>
                                        <p className="text-[13px] font-bold text-admin-value uppercase tracking-tight group-hover:text-red-400 transition-colors">{p.name}</p>
                                        <div className="flex items-center gap-2 mt-1">
                                            <span className="text-[9px] text-admin-dim font-bold uppercase tracking-widest">{p.category}</span>
                                            <span className="w-1 h-1 rounded-full bg-admin-dim/30" />
                                            <span className="text-[9px] text-admin-dim font-bold uppercase tracking-widest">{p.qty} sold</span>
            ...
            <Card 
                ...
                headerAction={
                    <div className="flex items-center gap-3">
                        ...
                        <button className="p-2 bg-card-bg border border-openpos-border rounded-xl text-admin-dim hover:text-openpos-blue transition-all">
                            <Filter size={14} />
                        </button>
                    </div>
                }
                    ...
                    <table className="w-full text-left">
                        ...
                        <tbody className="divide-y divide-openpos-border">
                            {[...topProducts, ...leastProducts].slice(0, 8).map((p) => (
                                <tr key={p.id} className="group hover:bg-openpos-bg-subtle/40 transition-colors cursor-default">
                                    <td className="p-5">
                                        <div className="flex flex-col">
                                            <span className="text-[13px] font-bold text-admin-value uppercase tracking-tight group-hover:text-openpos-blue transition-colors">{p.name}</span>
